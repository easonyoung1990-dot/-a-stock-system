...
// 用户的整套交易方法论（《投资 AI 大脑决策系统 v1》），以 ?raw 方式
// 把 markdown 原文件读成字符串，作为「我的方法论」的默认内容（首次打开即在）。
import METHODOLOGY_DEFAULT from './methodology.default.md?raw'
...
export function loadMethodology() {
  try {
    ...
    // 本地有非空内容就用本地的；否则回落到内置的《投资 AI 大脑决策系统》默认
    return saved && saved.trim() ? saved : METHODOLOGY_DEFAULT
  } catch {
